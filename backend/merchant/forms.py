...
class MerchantSignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['password1'].widget.attrs.update({'class': 'form-control', 'id': 'password', 'required': True, 'minlength': '8'})
        self.fields['password2'].widget.attrs.update({'class': 'form-control', 'id': 'confirmPassword', 'required': True})
...
class DeliverymanForm(forms.ModelForm):
    ...
    Address = forms.CharField(required=True, widget=forms.TextInput(attrs={'placeholder': 'Current address', 'id': 'address'}))
    Vehicle = forms.ChoiceField(required=True, choices=getattr(Deliveryman, 'VEHICLE_CHOICES', []), widget=forms.Select(attrs={'id': 'vehicletype'}))
    Zone = forms.ChoiceField(required=True, choices=getattr(Deliveryman, 'ZONE_CHOICES', []), widget=forms.Select(attrs={'id': 'deliveryzone'}))
    DutyTime = forms.ChoiceField(required=True, choices=getattr(Deliveryman, 'DUTYTIME_CHOICES', []), widget=forms.Select(attrs={'id': 'duty-time'}))
    VehicleNumber = forms.CharField(required=True, widget=forms.TextInput(attrs={'placeholder': 'BA 01 PA 1234 / 03-01-Pa-1234', 'id': 'vehicle-number'}))
    UserImage = forms.ImageField(required=True, widget=forms.ClearableFileInput(attrs={'id': 'identityimage', 'accept': 'image/*'}))
    PanNumber = forms.CharField(required=True, widget=forms.TextInput(attrs={'placeholder': '9 digit PAN number (123456789)', 'id': 'pan-number'}))
    DateofBirth = forms.DateField(required=True, widget=forms.DateInput(attrs={'type': 'date', 'id': 'dob'}))
    BillBookScanCopy = forms.FileField(required=True, widget=forms.ClearableFileInput(attrs={'id': 'billBook', 'accept': 'image/*,application/pdf'}))
